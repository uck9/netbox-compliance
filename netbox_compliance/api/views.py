...
from ..services import (
    ...
    record_package_report, record_result, score_device,
)
...
__all__ = (
    ...
    'CompliancePackageReportViewSet',
    'BulkResultIngestView',
    'PackageReportBulkIngestView',
...
class CompliancePackageReportViewSet(NetBoxModelViewSet):
    """Read-only + delete: reports are generated by an external evaluation run (see
    PackageReportBulkIngestView), never created/edited directly via this viewset."""
    queryset = CompliancePackageReport.objects.select_related('device', 'package')
    serializer_class = serializers.CompliancePackageReportSerializer
    filterset_class = filtersets.CompliancePackageReportFilterSet
    http_method_names = ['get', 'delete', 'head', 'options']
...
def _resolve_package(value):
    if value is None:
        return None
    if isinstance(value, int):
        return CompliancePackage.objects.filter(pk=value).first()
    return CompliancePackage.objects.filter(slug=value).first()
...
class PackageReportBulkIngestView(APIView):
    """
    POST /api/plugins/compliance/package-reports/bulk/

    Accepts a single {"device": ..., "reports": [...]} object, or a list of
    such objects to cover multiple devices in one call -- same single-or-
    list normalization as BulkResultIngestView. Each report item is
    {"package": <slug or pk>, "html": "<self-contained report>"}. Validates
    the whole payload before writing anything (atomic, no partial writes on
    error), same posture as BulkResultIngestView.
    """
    queryset = CompliancePackageReport.objects.all()
    ...
    def post(self, request):
        ...
        for batch_index, batch in enumerate(payload):
            ...
            source = batch.get('source', '')
            timestamp = _parse_timestamp(batch.get('timestamp'))
            if batch.get('timestamp') and timestamp is None:
                errors.append({'index': batch_index, 'error': f"Invalid timestamp: {batch.get('timestamp')!r}"})
                continue

            resolved_items = []
            for item_index, item in enumerate(batch.get('reports', [])):
                package = _resolve_package(item.get('package'))
                if package is None:
                    errors.append({
                        'index': batch_index, 'item': item_index,
                        'error': f"Unknown package: {item.get('package')!r}",
                    })
                    continue
                ...
                if not isinstance(html, str) or not html:
                    errors.append({
                        'index': batch_index, 'item': item_index, 'error': "Item must include a non-empty 'html' string.",
                    })
                    continue

                resolved_items.append({'package': package, 'html': html})

            resolved_batches.append({'device': device, 'source': source, 'timestamp': timestamp, 'items': resolved_items})
        ...
        with transaction.atomic():
            for batch in resolved_batches:
                for item in batch['items']:
                    # record_package_report() upserts -- a repost for a (device, package) pair
                    # already seen updates that row in place rather than erroring on the unique
                    # constraint.
                    record_package_report(
                        batch['device'], item['package'], html=item['html'],
                        source=batch['source'], timestamp=batch['timestamp'],
                    )
                    created += 1
